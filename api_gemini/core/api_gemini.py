...
def update_room_approval(room_id: str, approval_status: int, approval_content: list):
    ...
    try:
        ...
        # Chỉ cập nhật approval và modifieddate (bảng rooms không có cột approval_content)
        update_query = """
            UPDATE rooms 
...
def call_gemini_with_rotation(prompt_text: str, history: list = None, model="gemini-3-flash-preview"):
    """
...
{chr(10).join([f"- {os.path.basename(f)}" for f in downloaded_files])}
    ...
    models_to_try = [
        "gemini-3-flash-preview",    # Mới nhất 3.0 preview
        "gemini-2.5-flash",          # Tốt nhất cho vision
        "gemini-2.5-flash-lite",     # Nhẹ hơn
        "gemini-2.0-flash-lite",     # Nhanh, nhẹ nhất
    ]
